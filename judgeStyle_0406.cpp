...
int LoadXML(int N[3],string &Path,double &space);
...
//1 对于一条线超过6个点的点，取头和尾
void FromCoupleToCouple2(vector<vector<double> > &result_couple,vector<vector<double> > &result_couple2,double space,vector<vector<double> >& result_test);
//result_couple2输出文件
...
//***************辅助函数
void Result_couple2Output_part(vector<vector<double> > result_couple2);
//  临时函数
void Result_test_Output(vector<vector<double> > result_test);
//************************************主函数开始**************************************//
int main(int argc,char **argv)
{
	...
	double space;
	string Path;
	LoadXML(N,Path,space);
//	space /= 0.0254;	//长度单位由米转换为英寸
	...
	vector<vector<double> > result_couple2;	//存储坐标对，x,z,y1,y2...
	vector<vector<double> > XYplane;		//XOY平面的点坐标
	vector<vector<double> > result_test;	//0917，仅作测试用，用完删除
	Surface surf;
	...
	global_max.x = int(global_max.x / space + 1) * space;
	global_max.y = int(global_max.y / space + 1) * space;
	global_max.z = int(global_max.z / space + 1) * space;

	cout<<"dot.x,dot.y,dot.z is:"<<global_min.x<<" "<<global_min.y<<" "<<global_min.z<<endl;
	cout<<"dot.x,dot.y,dot.z is:"<<global_max.x<<" "<<global_max.y<<" "<<global_max.z<<endl;
	...
	FromCoupleToCouple2(result_couple,result_couple2,space,result_test);
	
//	cout<<"calc the XYplane_point and output to file..."<<endl;
	Find_Output_XYplane_Point(result_couple2,XYplane,N);

	Result_coupleOutputTofile(result_couple);		//输出result_couple结果，x0,y0,z0,z1,z2...
	Result_couple2OutputTofile2(result_couple2);	//输出result_cople2结果，一个x,z对应一个y（可以选择只输出某一平面）
//	Result_couple2OutputTofile(result_couple2,N);	//输出result_cople2结果，一个x,z对应一对y，没有表头
//	ResultOutputTofile(result);			// 最初的结果 result x,nor_x,y, nor_y, z,nor_z	输出 x z y

//  以下为临时函数（辅助或者测试函数）	
//	Result_couple2Output_part(result_couple2);		//需要手动改X,Y,Z坐标范围
//	Result_test_Output(result_test);	
	return 0;
...
{
	ostringstream name;
	name << "result_test"<<".dat";
	ofstream out(name.str().c_str());

	for(int i=0;i<result_test.size();i++)			//
	{
		for(int j=0;j<result_test[i].size();j++)
		{
			out<<result_test[i][j]<<"\t";
		}
		out<<endl;
...
void FindBoundaryPoint(double space,vector<string> PATH,vector<vector<double> > &result,Surface surf,Point global_min)
{
	...
	for(int j=0;j<pnum;j++)
	{
		...
		for(int i=0;i<tri_num;i++)
		{
			...
			if(normal.x > 1.e-4)			//若法向量x坐标大于0，则dir_symbol_x = 1;小于0：等于-1；等于0：等于0；
				dir_symbol_x = 1;
			else if(normal.x < -1.e-4)
				dir_symbol_x = -1;
			else
				dir_symbol_x = normal.x = 0;

			if(normal.y > 1.e-4)			//若法向量y坐标大于0，则dir_symbol_y = 1;小于0：等于-1；等于0：等于0；
				dir_symbol_y = 1;
			else if(normal.y < -1.e-4)
				dir_symbol_y = -1;
			else
				dir_symbol_y = normal.y = 0;

			if(normal.z > 1.e-4)			//若法向量z坐标大于0，则dir_symbol_z = 1;小于0：等于-1；等于0：等于0；
				dir_symbol_z = 1;
			else if(normal.z < -1.e-4)
				dir_symbol_z = -1;
			else
				dir_symbol_z = normal.z = 0;
			...
			//**************根据投影面选择循环顺序*****************//
			int xNum, yNum, zNum;
			xNum = int(max_tribox.x / space) - int(min_tribox.x / space) + 1;
			yNum = int(max_tribox.y / space) - int(min_tribox.y / space) + 1;
			zNum = int(max_tribox.z / space) - int(min_tribox.z / space) + 1;
			for(int k=0; k<=xNum; k++)
				for(int t=0; t<=zNum; t++)
					{
						dist_min = 50000.0;
						for(int s=0; s<=yNum; s++)	
						{
...
//								continue;
						...
						{
							latt.x = int((latt.x - global_min.x) / space +0.5);	//转换坐标部分
							latt.z = int((latt.z - global_min.z) / space +0.5);	//+0.1防止0.99在类型转换时出错
							//latt.y = (dist_min - global_min.y) / space;
							latt.y = int((temp_y - global_min.y) / space +0.5);	//垂直距离不用该句*********************
...
bool JudgePointProjectionOutOfTri_Y(Point p1,Triangle tri)
{
	...
	return (fabs(s_p1bc + s_p1ac + s_p1ab - s_tri) > 1.e-5);
}
...
bool JugdePointInTheTri(Point p1,Point tri_p,Point normal)
{
	...
	if(vec.x*normal.x + vec.y*normal.y + vec.z*normal.z >= -1.e-4)		//两向量点乘大于0，两向量同向，格点在三角形外部
		return false;
	else if(vec.x*normal.x + vec.y*normal.y + vec.z*normal.z < -1.e-4)	//！！！！！！！！没有考虑小于-0.00001这个情况********city在上表面出现这个情况，所以误删点
		return true;
...
bool comp(const vector<double> &p,const vector<double> &q)
{
	if(p[0] < q[0])
		...
		return true;
	else if(p[0] == q[0] && p[4] == q[4] && p[2] == q[2] && p[3] < q[3])	//防止单点的排序错误
		return true;
	else
		return false;
}
...
//////////////////////////////////////////////////////////////////////////
	//！！！！！！判断相等时候，假定点排列依据是先比较X，然后Z,最后Y！！！！！！！！！！ //
		...
		{
			if(fabs(result[i][2] - result[i+1][2]) < 1.e-10 && fabs(result[i][3] - result[i+1][3]) < 1.e-10)
			{
	...
	for(vector<vector<double> > ::iterator it = result_couple.begin();it != result_couple.end();it++)
	{
		...
		else
		{
			...
			{	
				...
				// //对于第一个点法向量是正的情况下，当作单点处理（实际情况可能是少了一面）
				// while((*it)[i+1] > 0 && i+1 < len)
				// {
				// 	point_temp.push_back((*it)[0]);
				// 	point_temp.push_back((*it)[1]);
				// 	point_temp.push_back((*it)[i]);
				...
				{
					if((*it)[i+1] > 0)
						cout<<"maybe there have some problem"<<(*it)[0]<<"\t"<<(*it)[1]<<"\t"<<(*it)[i]<<endl;

					point_temp.push_back((*it)[0]);			//x坐标
					point_temp.push_back((*it)[1]);			//z坐标
					point_temp.push_back((*it)[i]);

//					while(i+5 < len && (*it)[i+5] * (*it)[i+3] > 0)
//						i += 2;
				...
				else if((*it)[i+1] * (*it)[i+3] > 0 && len % 4 != 0 && i+3 < len)
				{
					if((*it)[i+1] > 0)
						cout<<"maybe there have some problem"<<(*it)[0]<<"\t"<<(*it)[1]<<"\t"<<(*it)[i]<<endl;
...
//之前方法的一路到底，暂时放弃。
	...
	cout<<"total num of single pair is :  "<<count<<endl;
}
...
//输出result_couple
//result_couple:存储坐标对，x,y,z0,f0,z1,f1,z2,f2.f为法向量
void Result_coupleOutputTofile(vector<vector<double> > result_couple)
{
...
//		out<<result_couple[i][0]<<"\t"<<result_couple[i][1]<<"\t";
		for(int j=2;j<result_couple[i].size();j = j+2)
		{
			out<<result_couple[i][0]<<"\t"<<result_couple[i][1]<<"\t";
			out<<result_couple[i][j]<<endl;  // 输出第一步边界点
//			out<<result_couple[i][j]<<"\t"<<result_couple[i][j+1]<<endl;	// 输出法向量
		}
	}
...
void Find_Output_XYplane_Point(vector<vector<double> > &result_couple2,vector<vector<double> > &XYplane,int N[3])
{
	...
	for(vector<vector<double> > ::iterator it = result_couple2.begin();it!=result_couple2.end();it++)
	{
		if((*it)[1] == zValue)
			...
			{
				out<<(*it)[0]<<"\t"<<i<<"\t"<<0<<"\t"<<124<<endl;				//坐标输出顺序为x,z,y				
			}
...
int LoadXML(int N[3],string &Path,double &space)
{
	...
	space = atof(pElement->GetText());

	pElement = pRoot->FirstChildElement("N3");
	pElement->QueryIntAttribute("NX",&N[0]);
	pElement->QueryIntAttribute("NY",&N[1]);
	pElement->QueryIntAttribute("NZ",&N[2]);
